...
const SKELETOR_SPAWN_INTERVAL = 8; // Time in seconds between each spawn
// sprites and images
...
scene('rules', () => {
  ...
  add([
    ...
    add([
        text('1. Use arrow keys and WASD to move', 14),
        origin('center'),
        pos(width() / 2, height() / 3),
        color(1, 0, 0) 
    ]);
    ...
    add([
        text('2. Run from the witch and collect book pieces', 10),
        origin('center'),
        pos(width() / 2, height() / 2),
        color(1, 0, 0) 
    ]);
    ...
    add([
        text('find the full book to go back home and fight', 10),
        origin('center'),
        pos(width() / 2, height() / 1.5),
        color(1, 0, 0) 
    ]);
    add([
      text('Press Space to continue', 10),
      origin('center'),
      pos(width() / 2, height() / 1.2),
      color(1, 0, 0) 
  ]);
    keyPress('space', () => {
        go('rules2', { level: 0, score: 0 });
    }); });
// Rules scene
scene('rules2', () => {
layers(['bg2', 'obj'], 'obj'); // 'obj' is the default layer
...
add([
  ...
  add([
      text('1. Use arrow keys & WASD to move', 16),
      origin('center'),
      pos(width() / 2, height() / 3),
      color(1, 0, 0) 
  ]);
  ...
  add([
      text('2. Avoid falling witch Spell and collect eggs', 10),
      origin('center'),
      pos(width() / 2, height() / 2),
      color(1, 0, 0) 
  ]);
  ...
  add([
      text('climb the platforms to take care of the witch and go back home', 7),
      origin('center'),
      pos(width() / 2, height() / 1.5),
      color(1, 0, 0) 
  ]);
...
//not moving add 'varname' like 'virus2' if remvove it moves
  ...
  add([sprite('bg'), layer('bg'), pos(105,-240)])
  ...
  if (level === 1) { // This ensures the boss only appears in level 2 (index 1)
    const bossLvl2 = add([
        sprite("boss2"),
        pos(700, height() - 0.1), // Adjust y-position to make the boss appear above the 8
        origin("bot"),
        { speed: 50, direction: 1 }, // direction: 1 means moving right, -1 means moving left
        scale(2),
        solid(),
        "dangerous2",
        "bossLvl2",
    ...
    const leftLimit = 0; // Adjust the left boundary as needed
    const rightLimit = 800; // Adjust the right boundary as needed

    // Boss movement logic
    bossLvl2.action(() => {
        // Move the boss based on its direction
        bossLvl2.pos.x += bossLvl2.speed * bossLvl2.direction * dt();

        // Reverse direction when the boss reaches the limits
        if (bossLvl2.pos.x <= leftLimit) {
            bossLvl2.direction = 1; // Change direction to right
        } else if (bossLvl2.pos.x >= rightLimit) {
            bossLvl2.direction = -1; // Change direction to left
        }

        
    });

    // Increment the speed gradually as time passes
    loop(1, () => {
        if (bossLvl2.speed < 100) {
            bossLvl2.speed += 1; // Adjust increment to control the rate of speed increase
        }
        
    });

    let spawnTimer = 0;
    const spawnInterval = 1; // Interval in seconds between enemy spawns

    bossLvl2.action(() => {
        // Increment the timer by the delta time each frame
        spawnTimer += dt();

        // Check if the spawn timer has reached the interval
        if (spawnTimer >= spawnInterval) {
            // Reset the timer
            spawnTimer = 0;

            // Ensure that the number of active enemies on screen does not exceed a limit
            const maxEnemies = 10; // Limit to 10 enemies at a time
            const currentEnemies = get('dangerous').length;

            if (currentEnemies < maxEnemies) {
                // Get bossLvl2's current position
                const bossPosX = bossLvl2.pos.x;

                // Spawn an enemy near bossLvl2's x position (with a random offset)
                add([
                    sprite('virus1'),
                    pos(bossPosX + rand(-50, 50), 0), // Random x offset near bossLvl2's position, y = 0 (top of the screen)
                    solid(),
                    scale(0.3),
                    'dangerous'
                ]);
    ...
    player.collides("bossLvl2", (boss) => {
        destroy(boss);
        {
          window.location.href = "win.html"; // Navigate to win.html
      }      });

    player2.collides("bossLvl2", (boss) => {
        destroy(boss);
        {
          window.location.href = "win.html"; // Navigate to win.html
      }
...
player.overlaps('win', (v) => {
window.location.href = 'win.html'
    destroy(k); // Destroy kaboom
});
player2.overlaps('win', (v) => {
  window.location.href = 'win.html'
      destroy(k); // Destroy kaboom
  });
keyPress('down', () => {
...
//pipe testing
  ...
  function spawnSkeletor() {
    const skeletor = add([
     sprite('space-invader'),// Add your skeletor sprite here
